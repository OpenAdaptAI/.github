...
PUBLICATION_RECOVERY_AUTHORIZATION_DOMAIN = (
    b"OpenAdapt production publication recovery authorization v1\0"
)
...
SUPPORT_ARTIFACT_INVENTORY_DOMAIN = (
    ...
    if target is not None:
        ...
        for artifact in result:
            ...
            if (
                ...
                or artifact["publish_destinations"] != list(destinations)
            ):
                raise TrustError(
...
def validate_staging(value: Any) -> dict[str, Any]:
    staging = closed(
        value,
        {
            ...
            "release_author_login", "assets", "immutable_releases",
            "immutable_releases_sha256", "tag_rulesets",
            "tag_rulesets_sha256", "tag_ref_state", "tag_ref_state_sha256",
            "observed_at",
        },
        "publication staging",
    )
    ...
    if (
        immutable_releases["enabled"] is not True
    ...
    tag_ref_state = closed(
        staging["tag_ref_state"], {"ref", "exists"}, "tag ref state"
    )
...
def validate_support_release(
    ...
    fields = {
        "schema_version", "admission_id_sha256", "lifecycle_state",
        "support_target", "verdict", "claim_scope", "release_identity",
        "release", "release_sha256", "artifact_inventory_sha256",
        "publication_staging", "publication_staging_sha256",
        "authority_state_sha256", "revocation_state_sha256",
        "signer_registry_sha256", "support_policy_sha256", "issued_at",
    ...
    release = closed(
        admission["release"],
        {
            "schema_version", "kind", "source_repository",
            "source_repository_id", "source_commit", "version", "tag",
            "deployment_id", "deployment_sha256", "artifacts",
        },
...
def validate_publication_recovery_authorization(
    ...
    for field in (
        "authorization_id_sha256", "qualification_release_object_sha256",
        "release_sha256", "artifact_inventory_sha256",
    ...
    if (
        ...
        or reference["object_schema_version"]
    ...
    expected_idempotency = "publication-recovery:" + hashlib.sha256(
        PUBLICATION_RECOVERY_IDEMPOTENCY_DOMAIN
        + canonical(idempotency_projection)
    ).hexdigest()
    ...
    if identity != digest_bytes(
        PUBLICATION_RECOVERY_AUTHORIZATION_DOMAIN, projection
    ):
...
def _validate_receipt_structure(
    ...
    fields = {
        ...
        "evidence_authority_contract_sha256", "campaign_permit_sha256", "signer_registry_sha256",
    ...
    campaign_summary = closed(
        ...
        {
            "schema_version", "minimum_trials_per_task_condition",
            "task_count", "classes",
        },
    ...
    if not isinstance(signature, str) or re.fullmatch(r"[A-Za-z0-9+/]{86}==", signature) is None:
    ...
    *,
    ...
    statement = closed(
        value.get("signing_statement"),
        {
            ...
            "unsigned_object_sha256", "unsigned_size_bytes", "commitment_scheme",
        },
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        result = subprocess.run(
            [
                ...
                "-inkey", str(key_path), "-rawin", "-in", str(statement_path),
...
def validate_authority_state(
    ...
    fields = {
        "schema_version", "authority_state_sha256", "status",
        "signer_registry_sha256", "signer_registry_identity_sha256",
        "signer_registry_revision", "evidence_authority_sha256", "observed_at",
        "not_before", "expires_at", "issuer_key_id", "algorithm",
        "signing_statement", "signature", "issuer",
    }
...
def validate_revocation_state(
    ...
    fields = {
        "schema_version", "revocation_state_sha256",
        "previous_revocation_state_sha256", "revision", "status",
        "authority_state_sha256", "signer_registry_sha256", "revocations",
        "observed_at", "not_before", "expires_at", "issuer_key_id", "algorithm",
        "signing_statement", "signature", "issuer",
    }
...
def _validate_release_identity(value: Any) -> dict[str, Any]:
    identity = closed(
        value,
        {
            "schema_version", "channel", "sequence",
    ...
    if (
        ...
        != "openadapt.monotonic-production-release/v1"
    ...
    if sequence == 1:
        if previous is not None:
            raise TrustError("the first release identity previous admission must be null")
    else:
...
def _validate_acceptance_issuer(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        or issuer["repository_owner_id"] != "132681217"
        or issuer["workflow"]
        != ".github/workflows/issue-production-acceptance.yml"
        or issuer["ref"] != "refs/heads/main"
...
def _validate_receipt_admission_binding(
    ...
    *,
    ...
    if admission["campaign_summary"] != receipt["campaign_summary"]["classes"]:
        raise TrustError("qualification admission campaign summary differs from the receipt")
...
def validate_acceptance_manifest(
    ...
    *,
    ...
    fields = {
        "schema_version", "target", "verdict", "claim_scope",
        "acceptance_policy_sha256", "lifecycle_policy_sha256",
        "release_identity", "release", "release_sha256", "artifact_inventory",
        ...
        "qualification_admission_reference", "qualification_admission_bundle_reference",
        "campaign_summary", "authority_state_sha256", "revocation_state_sha256",
        "signer_registry_sha256", "issued_at", "not_before", "expires_at", "issuer",
    }
    manifest = closed(value, fields, "production acceptance manifest")
    if (
        manifest["schema_version"] != "openadapt.production-acceptance/v2"
    ...
    release = closed(
        manifest["release"],
        {
            ...
            "source_commit", "version", "tag", "deployment_id",
            "deployment_sha256", "artifacts",
        },
    ...
    if (
        ...
        or manifest["artifact_inventory_sha256"]
    ...
    if (
        ...
        or manifest["campaign_summary"]
        != bound_receipt["campaign_summary"]["classes"]
        or manifest["signer_registry_sha256"]
        != bound_receipt["signer_registry_sha256"]
...
def acceptance_summary_identity(value: Mapping[str, Any]) -> str:
    projection = {
        ...
        for field in (
            ...
            "artifact_inventory_sha256", "publication_staging_sha256",
            "qualification_evidence_decision_receipt_reference",
            "qualification_admission_reference",
            "production_acceptance_manifest_reference", "campaign_summary",
            "authority_state_sha256", "revocation_state_sha256",
            "signer_registry_sha256", "issuer",
        )
    }
...
def validate_acceptance_summary(
    ...
    *,
    ...
    fields = {
        "schema_version", "target", "verdict", "claim_scope",
        "acceptance_policy_sha256", "lifecycle_policy_sha256",
        "release_identity", "release_sha256", "artifact_inventory_sha256",
        "publication_staging", "publication_staging_sha256",
        ...
        "production_acceptance_manifest_bundle_reference", "campaign_summary",
        "authority_state_sha256", "revocation_state_sha256", "signer_registry_sha256",
        "issued_at", "not_before", "expires_at", "issuer",
    }
    ...
    if (
        ...
        or summary["claim_scope"]
        != TARGET_CONTRACTS[summary["target"]]["claim_scope"]
    ):
    ...
    for field in (
        "target", "claim_scope", "acceptance_policy_sha256",
        "lifecycle_policy_sha256", "release_identity", "release_sha256",
        "artifact_inventory_sha256", "publication_staging",
        "publication_staging_sha256",
        "qualification_evidence_decision_receipt_reference",
        "qualification_evidence_decision_receipt_bundle_reference",
        ...
        if summary[field] != bound_manifest[field]:
            raise TrustError(f"production acceptance summary {field} differs from the manifest")
...
def validate_release_evidence_chain(
    ...
    *,
    ...
    for field in (
        "target", "claim_scope", "release_identity", "release_sha256",
        "artifact_inventory_sha256", "publication_staging",
        "publication_staging_sha256", "authority_state_sha256",
        "revocation_state_sha256", "signer_registry_sha256",
    ):
        if release[field] != bound_summary[field]:
            raise TrustError(f"release admission {field} differs from its evidence chain")
    ...
    if (
        ...
        or bound_manifest["artifact_inventory"]["claim_scope"]
        != release["claim_scope"]
    ...
    if release["publication_policy_sha256"] != bound_summary["acceptance_policy_sha256"]:
...
def validate_cloud_deploy_authorization(
    ...
    fields = {
        "schema_version", "authorization_sha256", "deployment_intent_sha256",
        "profile_repository", "profile_repository_id", "profile_commit",
        "profile_workflow", "profile_run_id", "profile_run_attempt",
        "cloud_source_commitment_sha256", "expected_live_attestation_sha256",
        "provider_idempotency_key", "audience", "signer_registry_sha256",
        "revocation_state_sha256", "issued_at", "not_before", "expires_at", "issuer",
    }
    ...
    if (
        ...
        or re.fullmatch(r"cloud-deploy:[0-9a-f]{64}", authorization["provider_idempotency_key"])
        is None
    ):
...
def validate_cloud_deployment_handoff(
    ...
    fields = {
        "schema_version", "handoff_id_sha256", "authorization_reference",
        "authorization_bundle_reference", "authorization_sha256",
        "cloud_source_commitment_sha256", "profile_commit", "profile_run_id",
        "profile_run_attempt", "source_proof_request", "source_proof_request_sha256",
        "source_proof_response", "source_proof_response_sha256",
        "reviewed_public_values_sha256",
        "expected_live_attestation_sha256", "live_attestation_sha256",
        "provider_idempotency_key", "audience", "signer_registry_sha256",
        "revocation_state_sha256", "verdict", "issued_at", "not_before",
        "expires_at", "issuer",
    }
    ...
    request_fields = {
        "schema_version", "authorization_sha256", "cloud_source_commit",
        "expected_live_attestation_sha256", "expires_at", "issued_at", "nonce",
        "production_origin", "profile_commit", "profile_environment",
        "profile_repository", "profile_workflow_ref", "run_attempt", "run_id",
        "site_id", "source_names",
    }
    request = closed(handoff["source_proof_request"], request_fields, "Cloud source proof request")
    if (
    ...
    try:
        nonce_bytes = evidence.base64.urlsafe_b64decode(
            nonce + "=" * (-len(nonce) % 4)
        )
    except Exception as exc:
    ...
    if (
        len(nonce_bytes) != 32
        or evidence.base64.urlsafe_b64encode(nonce_bytes).decode().rstrip("=")
        != nonce
    ):
    ...
    if (
        ...
        or source_names != list(CLOUD_LIVE_SOURCE_NAMES)
    ):
    ...
    expected_idempotency = "cloud-deploy:" + hashlib.sha256(
        CLOUD_PROVIDER_IDEMPOTENCY_DOMAIN
        + canonical(
            {
    ...
    response_fields = {
        "schema_version", "request_sha256", "authorization_sha256",
        "live_attestation_sha256", "live_deployment", "oidc_identity_sha256",
        "production_origin", "proofs", "public_values", "site_id",
    }
    response = closed(handoff["source_proof_response"], response_fields, "Cloud source proof response")
    if response["schema_version"] != "openadapt.production-secret-source-proof-response/v2":
    ...
    if not isinstance(proofs, dict) or sorted(proofs) != list(CLOUD_SECRET_SOURCE_NAMES):
    ...
    if handoff["source_proof_response_sha256"] != "sha256:" + hashlib.sha256(canonical(response)).hexdigest():
        raise TrustError("Cloud source proof response digest differs")
    equalities = {
        "authorization_sha256": handoff["authorization_sha256"].removeprefix(
            "sha256:"
        ),
    ...
    if (
        "sha256:" + response["request_sha256"]
        != handoff["source_proof_request_sha256"]
...
def validate_checkpoint_expiry_containment(
    ...
    *,
    ...
    workflows = [
        validate_qualification_admission(item)
        for item in workflow_admissions
    ]
    ...
    for child_not_before, child_expires, child_issued, label in windows:
        ...
        if checkpoint_not_before < child_not_before or checkpoint_expires > child_expires:
...
def validate_feed_expiry_containment(
    ...
    *,
    ...
    if now is not None:
        active = [
            index
            for index, (not_before, checkpoint_expires) in enumerate(
                checkpoint_windows
            )
    ...
    if (
        ...
        or current_reference["registry_source_commit"]
        != feed["registry_source_commit"]
        or current_reference["registry_revision"] != feed["registry_revision"]
        or current_reference["registry_head_sha256"]
        != feed["registry_head_sha256"]
    ):

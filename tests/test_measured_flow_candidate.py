...
def manifest(tmp_path, *, wheel=None, version="1.35.0"):
    # These are explicit parser fixtures, not evidence that a runtime executed.
    if wheel is None:
...
def test_incomplete_input_writes_refusal_candidate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "artifacts": [
                {"kind": "python-wheel", "sha256": value["runtime"]["wheel_sha256"]}
            ]
        },
    )
...
@pytest.mark.parametrize("versions", [("1.35.1", "1.35.0"), ("1.35.0", "1.35.0")])
def test_wrong_unpublished_package_versions_refuse(tmp_path, versions):
    wheel, sdist = archives(tmp_path, *versions)
    with pytest.raises(ValueError):
        candidate.observe_unpublished(wheel, sdist, "c" * 40)


def test_unpublished_candidate_cannot_assert_admission_or_publication(
...
def test_partial_unpublished_inputs_refuse_before_network_observation(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(
        candidate.sys,
    ...
    monkeypatch.setattr(
        candidate, "observe_release", lambda: pytest.fail("unexpected network request")
    )
...
def test_rehashed_observation_cannot_relabel_a_failed_trial(tmp_path):
    path, value = manifest(tmp_path)
    ...
    path.write_text(json.dumps(value))
    with pytest.raises(ValueError, match="retained observation differs"):
        candidate.summarize(path, value["runtime"]["wheel_sha256"])
...
def test_class_events_cannot_be_concentrated_in_one_trial(tmp_path, class_name, field):
    path, value = manifest(tmp_path)
    ...
    for trial, count in zip(selected, [3, 0, 0]):
        ...
        update_references(value, observed_path)
    path.write_text(json.dumps(value))
    with pytest.raises(ValueError, match="each trial must satisfy"):
        candidate.summarize(path, value["runtime"]["wheel_sha256"])


def test_attended_decision_completion_cannot_replace_native_verified_effect(tmp_path):
    path, value = manifest(tmp_path)
    report_path = tmp_path / "declared_attended-0-report.json"
    report = json.loads(report_path.read_text())
    report.update(
        success=False, transaction_outcome="COMPLETED_UNVERIFIED", report_success=True
    )
    report_path.write_text(json.dumps(report))
    ...
    update_references(value, observed_path)
    path.write_text(json.dumps(value))
    with pytest.raises(ValueError, match="primary native outcome"):
        candidate.summarize(path, value["runtime"]["wheel_sha256"])


def test_rehashed_native_report_must_match_normalized_claim(tmp_path):
    path, value = manifest(tmp_path)
    ...
    path.write_text(json.dumps(value))
    with pytest.raises(
        ValueError, match="native report differs from observed model_calls"
    ):
        candidate.summarize(path, value["runtime"]["wheel_sha256"])


def test_installed_proof_cannot_omit_a_wheel_member(tmp_path):
    path, value = manifest(tmp_path)
    proof_path = tmp_path / "installed.json"
    ...
    for trial in value["trials"]:
        ...
        update_references(value, observed_path)
    path.write_text(json.dumps(value))
    with pytest.raises(ValueError, match="installed member proof does not cover"):
        candidate.summarize(path, value["runtime"]["wheel_sha256"])


@pytest.mark.parametrize("reference", [None, {}, {"path": "", "sha256": ""}])
def test_missing_or_empty_normalizer_reference_refuses(tmp_path, reference):
    path, value = manifest(tmp_path)
    ...
    path.write_text(json.dumps(value))
    with pytest.raises(ValueError, match="measurement normalizer is absent"):
        candidate.summarize(path, value["runtime"]["wheel_sha256"])
